...
def GetLogs(srvname, srvtype, fromdate, todate, status):
    result = session.query(PredLog).filter(PredLog.servicetype == srvtype).filter(PredLog.servicename == srvname).filter(PredLog.createdon >= fromdate).filter(PredLog.createdon <= todate).filter(PredLog.status == status.upper()).order_by(PredLog.createdon.desc()).all()
    return result
